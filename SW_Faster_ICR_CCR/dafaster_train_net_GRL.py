...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            ################# add grl code ##################
            dloss_t_grl=0.5*FL(projection_grl,domain_t)

            if args.dataset == "sim10k":
                loss += (dloss_s + dloss_t + dloss_s_p + dloss_t_p + dloss_t_grl) * args.eta
            else:
                loss += dloss_s + dloss_t + dloss_s_p + dloss_t_p + dloss_t_grl
            ...
            if step % args.disp_interval == 0:
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f eta: %.4f dloss_s_grl: %.4f dloss_t_grl: %.4f"
                    % (
