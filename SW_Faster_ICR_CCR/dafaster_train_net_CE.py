...
def parse_args():
  ...
  """
    ...
    # parser.add_argument(
    #     "--lr", dest="lr", help="starting learning rate", default=1e-04, type=float
    # )
    parser.add_argument(
        "--lr_decay_step",
        dest="lr_decay_step",
    ...
    parser.add_argument(
        "--ce",
        dest="ce",
        help="whether change local l2 loss -> cross entropy loss",
        action="store_true",
    )
    # parser.add_argument(
    #     "--da_use_contex",
    ...
    parser.add_argument(
        "--grl",
        dest="grl",
        help="whether add grl for projection feature map",
        action="store_true",
    )
...
if __name__ == "__main__":
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            ################# add grl code ##################
            if args.grl:
                domain_s_grl = Variable(torch.zeros(projection_grl.size(0)).long().cuda())
                dloss_s_grl = 0.5 * FL(projection_grl, domain_s_grl)
                # print(domain_s.shape)
                # print(domain_s_grl.shape)

            # local alignment loss
            if args.lc:
                dloss_s_p = 0.5 * torch.mean(out_d_pixel ** 2)

            # source local Change Loss --> Cross Entropy
            if args.ce:
                local_criterion_s = nn.CrossEntropyLoss().cuda()
                dloss_s_p = 0.5 * local_criterion_s(out_d_pixel, domain_s)
            ...
            if args.grl:
                out_d_pixel, out_d, d_pixel_grl, projection_grl = fasterRCNN(
                    im_data,
                    im_info,
                    # im_cls_lb, #Megvii
                    gt_boxes,
            ...
            # print(out_d_pixel.shape)  # torch.Size([45000, 1])

            # local alignment loss
            if args.lc:
                # loss for local.
                dloss_t_p = 0.5 * torch.mean((1 - out_d_pixel) ** 2)
            # target global Change Loss --> Cross Entropy
            # ****************************************************
            if args.ce:
                local_criterion_t = nn.CrossEntropyLoss().cuda()
                # dloss_t_p_ce = 0.5 * local_criterion_t(out_d_pixel, domain_t)
                dloss_t_p = 0.5 * local_criterion_t(out_d_pixel, domain_t)

            if args.grl:
                domain_t_grl = Variable(torch.ones(projection_grl.size(0)).long().cuda())
                dloss_t_grl = 0.5 * FL(projection_grl, domain_s_grl)
