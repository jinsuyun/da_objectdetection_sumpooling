...
class netD_grl(nn.Module):
    def __init__(self, context=False):
        super(netD_grl, self).__init__()

        self.conv1 = conv1x1(512, 512)
        self.bn1 = nn.BatchNorm2d(512)
        self.conv2 = conv1x1(512, 128)
        self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = conv1x1(128, 128)
        self.bn3 = nn.BatchNorm2d(128)
        self.fc = nn.Linear(128, 2)
...
class vgg16(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        self.netD_pixel_CE = netD_pixel_CE(context=self.ce)
        self.netD_grl = netD_grl(context=self.grl)
        feat_d = 4096
        if self.lc:
            feat_d += 128
        if self.gc:
