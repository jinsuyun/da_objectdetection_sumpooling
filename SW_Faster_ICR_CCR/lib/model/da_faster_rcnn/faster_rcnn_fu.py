...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic, lc, gc, ce, grl, ct):
    ...
    def forward(
        ...
        if self.ce:
            ...
            # d_pixel = self.netD_pixel_CE(base_feat1)  # remove GRL
        ...
        if self.lc:
            ...
            if not target:
                if self.ct:
                    ...
                    feat_pixel = self.netD_pixel(base_feat1.detach())

        elif not self.lc and not self.ce:
            d_pixel = self.netD_pixel(grad_reverse(base_feat1, lambd=eta))
        ...
        if self.gc:
            if self.ct:
                ...
                if self.ct:
                    ...
                    feat = self.netD(base_feat.detach())
        else:
            domain_p = self.netD(grad_reverse(base_feat, lambd=eta))
            if target:
                return d_pixel, domain_p  # ,diff
        ...
        ################# add grl code ##################
        if self.grl:
            projection = torch.sum(base_feat)
            projection_grl, _ = self.netD(grad_reverse(base_feat, lambd=eta))
            if target:
                return d_pixel, projection_grl  # d_pixel이 맞나?
            _, feat_grl = self.netD(base_feat.detach())
        else:
            projection_grl = self.netD(grad_reverse(base_feat, lambd=eta))
            if target:
                return d_pixel, projection_grl
