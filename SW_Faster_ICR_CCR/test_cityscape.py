...
for i in range(start_epoch, max_epochs + 1):
    ...
    # )
    command = "python eval/test_strong_weak.py  --lc --cuda --gc --part {} --net {} --dataset {} --model_dir {} --output_dir {} --num_epoch {}".format(
        part, net, dataset, model_dir, output_dir, i
    )

    # command = "python eval/test_strong_weak.py --cuda --gc --lc --part {} --net {} --dataset {} --model_dir {} --output_dir {} --num_epoch {}".format(
    #     part, net, dataset, model_dir, output_dir, i
    # )
    os.system(command)
