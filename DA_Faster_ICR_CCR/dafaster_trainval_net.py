...
def parse_args():
  ...
  """
    ...
    # parser.add_argument(
    #     "--lr", dest="lr", help="starting learning rate", default=0.01, type=float
    # )
    parser.add_argument(
        "--lr_decay_step",
        dest="lr_decay_step",
...
if __name__ == "__main__":
    ...
    if args.net == "vgg16":
        ...
        fasterRCNN = resnet(
            s_imdb.classes, 101, pretrained=True, class_agnostic=args.class_agnostic, net=args.net,
            model_path=args.pretrained_path
            # ,grl=args.grl
        )
    elif args.net == "res50":
        fasterRCNN = resnet(
            s_imdb.classes, 50, pretrained=True, class_agnostic=args.class_agnostic, net=args.net,
            model_path=args.pretrained_path
            # ,grl=args.grl
        )
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        start_steps = epoch * len(s_dataloader)
        total_steps = args.max_epochs * len(t_dataloader)
        ...
        for step in range(iters_per_epoch):
            ...
            if args.grl:
                loss = (
                        ...
                        * (
                            ...
                            + DA_img_loss_cls_grl.mean()
                            + tgt_DA_img_loss_cls_grl.mean()
                            + DA_cst_loss.mean()  # Consistency
                            + tgt_DA_cst_loss.mean()  # Consistency
                            # + (0 * DA_cst_loss.mean())  # Consistency 0 처리
                            # + (0 * tgt_DA_cst_loss.mean())  # Consistency 0 처리
                            + (alpha * matmul_DA_cst_loss.mean())
                            + (alpha * matmul_tgt_DA_cst_loss.mean())
                        )
                )
            else:
                loss = (
            ...
            if step % args.disp_interval == 0:
                ...
                if args.grl:
                    loss_DA_img_cls_grl = (
                            args.lamda
                            * (DA_img_loss_cls_grl.item() + tgt_DA_img_loss_cls_grl.item())
                            / 2
                    )

                    matmul_cst_loss = (
                            alpha*args.lamda
                            * (matmul_DA_cst_loss.item() + matmul_tgt_DA_cst_loss.item())
                            / 2
                    )
                else:
                    loss_DA_img_cls_grl = 0
                    matmul_cst_loss = 0
                ...
                print(
                    "\t\t\tcategory_cls: %.4f, rpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\timg_loss %.4f,ins_loss %.4f,,cst_loss %.4f, loss_DA_img_cls_grl %.4f, loss_matmul_cst %.4f"
                    % (
                        loss_category_cls,
                        loss_rpn_cls,
                        loss_rpn_box,
